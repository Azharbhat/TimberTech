...
export default function BoxBuyers({ route, navigation }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const snapshot = await get(ref(database, `Mills/${key}/BoxBuyers`));
        if (snapshot.exists()) {
          const keys = [];
  ...
  const handleItemPress = (item, workerKey) => {
    navigation.navigate('BoxBuyerDetails', { key: key, workerKey: workerKey, data: item });
  ...
  return (
    <View style={styles.container}>
      ...
      {showAddWorker && (
        ...
        <TouchableOpacity onPress={() => { navigation.navigate('AddBoxBuyers', { key: key }) ,setShowAddWorker(false)}}>
          <Text style={styles.addWorkerButtonn}>Add</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => { navigation.navigate('DeleteWorker', { key: key,address:'BoxBuyers' }),setShowAddWorker(false) }}>
          <Text style={styles.addWorkerButtonn}>Delete</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => { navigation.navigate('CopyId', { key: key,address:'BoxBuyers' }),setShowAddWorker(false) }}>
          <Text style={styles.addWorkerButtonn}>Get Id</Text>
        </TouchableOpacity>
        </View >
