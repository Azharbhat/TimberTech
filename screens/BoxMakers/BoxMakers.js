...
export default function BoxMakers({ route, navigation }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const snapshot = await get(ref(database, `Mills/${key}/BoxMakers`));
        if (snapshot.exists()) {
          const keys = [];
  ...
  const handleItemPress = (item, workerKey) => {
    navigation.navigate('BoxMakerDetail', { key: key, workerKey: workerKey, data: item });
  ...
  return (
    <View style={styles.container}>
      ...
      {showAddWorker && (
        ...
        <TouchableOpacity onPress={() => { navigation.navigate('DeleteWorker', { key: key ,address:'BoxMakers' }) ,setShowAddWorker(false)}}>
